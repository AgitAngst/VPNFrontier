...
function doCheck(){
  ...
  chrome.runtime.sendMessage({action:"recheckNow"},function(state){
    ...
    else if(state.analysisType==="foreign"){showResult("fail",t("bl.fail_title"),t("bl.fail_detail",{country:countryFull(state.country)}));playBlockSound()}
    else{showResult("fail",t("bl.unk_title"),t("bl.unk_detail"));playBlockSound()}
  });
}
...
document.getElementById("btn-back").addEventListener("click",function(){if(history.length>1)history.back();else chrome.tabs.getCurrent(function(tab){if(tab)chrome.tabs.update(tab.id,{url:"chrome://newtab"})})});
