// VPN Frontier v6.1.0-android — Background (Firefox Android)
// lang.js loaded via manifest scripts array
...
var DEFAULT_FREQ = 10;
// Android: no "focus" trigger (no windows.onFocusChanged)
var DEFAULT_COND = ["nav","timer","startup"];
...
async function trackBlock(url){
  ...
  var today=d[key]||0;var total=d.statsTotal||0;var domains=d.statsDomains||{};
...
// ── Badge (works on Android Firefox) ──
...
async function fetchGeo(){
  ...
  for(var i=0;i<active.length;i++){
    var pv=active[i];
    try{
      var r=await fetch(pv.url,{headers:{Accept:"application/json"},signal:AbortSignal.timeout(8000)});
      if(!r.ok)continue;
...
async function doFreshCheck(){
  ...
  activeCheck=(async function(){
    try{
      ...
      // Store alert for VPN detection (no system notifications on Android)
      var prev=await chrome.storage.local.get(["analysisType","country"]);
      var alert=null;
      if(a.type==="ru_vpn"&&prev.analysisType!=="ru_vpn"){
        alert={type:"vpn",title:t("n.vpn_title"),msg:t("n.vpn_msg",{reason:a.reason}),time:Date.now()};
      }
      if(prev.country&&prev.country!==(geo&&geo.country)){
        alert={type:"country",title:t("n.country_title"),msg:t("n.country_msg",{from:prev.country,to:geo?geo.country:"?",status:shouldBlock?t("n.block_on"):t("n.block_off")}),time:Date.now()};
      }
      ...
      if(alert)state.lastAlert=alert;
...
chrome.alarms.onAlarm.addListener(async function(a){
  ...
  if(conds.indexOf("wake")!==-1){
    ...
    if(actual>expected*1.5)doFreshCheck();
  }
});
...
chrome.runtime.onMessage.addListener(function(msg,sender,reply){
  switch(msg.action){
    ...
    case"getState":
      (async function(){var l=await chrome.storage.local.get(null);var s=await getSync();reply(Object.assign({},l,s,{builtinTlds:BUILTIN_TLDS,allProviders:ALL_PROVIDERS.map(function(p){return{id:p.id,name:p.name}}),platform:"android"}))})();return true;
    case"togglePause":
    ...
    case"proceedToUrl":
      (async function(){try{var u=new URL(msg.url);if(u.protocol!=="http:"&&u.protocol!=="https:"){reply({ok:false});return}chrome.tabs.update(sender.tab.id,{url:msg.url})}catch(e){}reply({ok:true})})();return true;
    case"addCustomTld":
    ...
    case"setLang":
      (async function(){await chrome.storage.sync.set({lang:msg.lang});currentLang=msg.lang;reply({ok:true})})();return true;
    case"setProviders":
    ...
    case"resetStats":
      (async function(){
        ...
        await chrome.storage.local.set({statsTotal:0,statsDomains:{}});
        for(var i=0;i<removes.length;i++)await chrome.storage.local.remove(removes[i]);
        reply({ok:true});
      })();return true;
    case"dismissAlert":
      chrome.storage.local.remove("lastAlert",function(){reply({ok:true})});return true;
  }
});
